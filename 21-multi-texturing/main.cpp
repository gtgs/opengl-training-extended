...
GLuint loadTexture(const char *filename, GLuint tu, GLuint format, int width, int height, GLint bitsPerPixel);
...
int main()
{
	...
	GLuint vertexShader = loadShaderFromFile("./assets/shaders/m_v_p_uv_light_mt_vs.glsl", GL_VERTEX_SHADER);
	if (!vertexShader)
	{
	...
	GLuint fragmentShader = loadShaderFromFile("./assets/shaders/m_v_p_uv_light_mt_fs.glsl", GL_FRAGMENT_SHADER);
	if (!vertexShader)
	{
	...
	int height = 0;
	int bpp = 0;
	GLuint map = loadTexture("./assets/textures/cubemap.png", GL_TEXTURE0, GL_RGBA, width, height, bpp);
	if (map == 0)
	{
	...
	GLuint mapBrick = loadTexture("./assets/textures/brick.png", GL_TEXTURE1, GL_RGB, width, height, bpp);
	if (mapBrick == 0)
	{
		fprintf(stderr, "could not load texture");
		return EXIT_FAILURE;
	...
	GLuint mapTile = loadTexture("./assets/textures/tiles.bmp", GL_TEXTURE2, GL_RGB, width, height, bpp);
	if (mapTile == 0)
	{
		fprintf(stderr, "could not load texture");
		return EXIT_FAILURE;
	...
	do
	{
		...
		GLuint uniformMap = glGetUniformLocation(programId, "map");
		GLuint uniformMap2 = glGetUniformLocation(programId, "map2");
		...
		glBindTexture(GL_TEXTURE_2D, mapTile);
		...
		glUniform1i(uniformMap, 1);
		glUniform1i(uniformMap2, 2);
